...
def _gen_latex(texcode, stdout, stderr, type='pdf'):
    ...
    try:
        ...
        raise Exception('Could not read log file; something has gone horribly wrong. Error details: %s' % e)
    ...
    if type == 'log':
        ...
        raise Exception('LaTeX generated no output.  Are you sure you selected any users?')

    if type == 'svg':
        retcode = call(['inkscape', '%s.pdf' % file_base, '-l', '%s.svg' % file_base])
    elif type == 'png':
        retcode = call(['convert', '-density', '192', '%s.pdf' % file_base, '%s.png' % file_base])

    if retcode:
        raise Exception("Postprocessing failed; try downloading as PDF.")
